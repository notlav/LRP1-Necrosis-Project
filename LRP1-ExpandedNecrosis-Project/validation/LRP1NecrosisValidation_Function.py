# Lavie Ngo
# May 2025
# LRP1 paper validation figures - function version
...
def runValidation(data):
    ...
    species_map = {speciesIDs[i]: i for i in range(len(speciesIDs))}
    reaction_map = {f'r{i+1}': i for i in range(len(w))}
    ...
    output_index = [species_map[o] for o in output]
    ...
    for i in range(len(inputCode)):
        ...
        # Prepare locals dict
        exec_locals = {**species_map, **reaction_map, 'w': w, 'tau': tau, 'y0': y0, 'ymax': ymax, 'n': n, 'EC50': EC50}
        exec(inputCode.iloc[i], globals(), exec_locals)
